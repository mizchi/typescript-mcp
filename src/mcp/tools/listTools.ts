...
export function createLanguageListTool(
  ...
  const languageTools: ToolInfo[] = [
    // LSP tools (using Language Server Protocol)
    {
      ...
      description:
        `Get hover information (type signature, documentation) for ${displayName} using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Find all references to a ${displayName} symbol across the codebase using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
    ...
    },
    {
      ...
      description:
        `Get ${displayName} diagnostics (errors, warnings) for a file using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Rename a ${displayName} symbol across the codebase using Language Server Protocol`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Delete a ${displayName} symbol and optionally all its references using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
    ...
    },
    {
      ...
      description:
        `Search for symbols across the entire ${displayName} workspace using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Get code completion suggestions in a ${displayName} file using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Get signature help for ${displayName} function calls using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Get available code actions for ${displayName} code using LSP`,
      category: "lsp",
      requiresLSP: true,
    },
    {
      ...
      description:
        `Format a ${displayName} document using the language server's formatting provider`,
      category: "lsp",
      requiresLSP: true,
    },
  ];
  ...
  return {
    ...
    description:
      `List all available ${displayName} MCP tools with descriptions and categories`,
    schema,
    execute: async ({ category = "all" }) => {
...
function formatLanguageToolsList(
  tools: ToolInfo[],
  category: string,
  displayName: string,
): string {
  ...
  if (filteredTools.length > 0 && (category === "all" || category === "lsp")) {
    ...
    result += "These tools require a running Language Server.\n";
    result +=
      `Make sure the appropriate LSP server is installed for ${displayName}.\n\n`;
  ...
  result += "## 💡 Tips\n";
  result +=
    `- All ${displayName} tools use the Language Server Protocol (LSP)\n`;
  result +=
    "- Get help for any tool: use the tool with no parameters to see its schema\n";
  result +=
    `- Make sure your ${displayName} LSP server is properly installed and running\n`;
